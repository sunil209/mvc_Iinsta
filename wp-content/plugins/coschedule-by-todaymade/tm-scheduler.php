...
if ( ! class_exists( 'tm_coschedule' ) ) {
    ...
    class TM_CoSchedule {
        ...
        public function get_cache_bust() {
            ...
            $unix_length = 13; // Length of unix timestamp in milliseconds

            // Validate response
            if ( true === is_string( $response ) ) {
                $result = $response;
            } elseif ( true === is_array( $response ) && true === isset( $response['body'] ) && is_string( $response['body'] ) && strlen( $response['body'] ) === $unix_length ) {
