...
final class CPRO_Service_Sendy extends CPRO_Service {
	 ...
	 * @var string $id
	 */
	public static $mapping_fields = array( 'NAME', 'GDPR' );
	...
	public function subscribe( $settings, $email ) {
		...
		if ( ! $api ) {
				...
				if ( 'email' !== $key && 'date' !== $key ) {
					if ( 'custom_field' === $settings['meta'][ $key ] ) {
						...
						if ( 'NAME' === $settings['meta'][ $key ] ) {
							$name = $p;
						} elseif ( 'GDPR' === $settings['meta'][ $key ] ) {
							$custom_field['gdpr'] = 'true';
						}
					}
				}
			}
			...
			if ( function_exists( 'cpro_get_user_location' ) ) {
				$location_data           = cpro_get_user_location();
				$custom_field['country'] = $location_data['country_code'];
			}

			$custom_field['ipaddress'] = $ip;
			$custom_field['referrer']  = $_SERVER['HTTP_REFERER'];
